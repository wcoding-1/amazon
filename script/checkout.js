import {cart,removeFromCart,displayTotalProducts,updateCartItem} from '../data/cart.js';
import {products} from '../data/products.js';
import dayjs from 'http://unpkg.com/dayjs@1.11.10/esm/index.js';
import { formatCurrency } from '../utils/util.js';
import {deliveryOption} from '../data/deliveryDate.js'
...
const days = dayjs()
let toDay = days.add(date.deliveryDays, 'day');
const dayFormat = toDay.format('YYYY-MMMM-D');
    ...
    js-cart-container-${productObj.id}">
                    ...
                    <div class="checkout-option">
                        ...
                        <form>
                            ${deliveryOptionDays(productObj.id, cartItem)}
                        </form>
                    </div>
...
//delivery option
function deliveryOptionDays(productId, cart) {
    let optionHTML = '';
    ...
    deliveryOption.forEach(option =>{
        ...
        const isChecked = option.id === cart.deliveryDateId

       optionHTML += `
        <div>
            <input type="radio" name="option-1"
            ${isChecked ? 'checked': ''}
            class="js-option-${productId}">
